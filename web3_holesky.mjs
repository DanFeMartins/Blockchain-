...
async function main() {
    ...
    async function cadastrarCliente() {
        try {
            ...
            const receipt = await sendTransaction(method, { value: web3.utils.toWei('0.0001', 'ether') });
    ...
    async function getCliente() {
        try {
            ...
            console.log(`Comorbidades: ${cliente[1]}`);
            console.log(`Alergias: ${cliente[2]}`);
            console.log(`Tipo Sanguíneo: ${cliente[3]}`);
            console.log(`restricao: ${cliente[4]}`);
        } catch (error) {
